...
**Dispatch routing (stays on /compat — matches upstream):** `pi-model-caller.ts`/`compaction.ts`/`branch-summarization.ts` (`completeSimple`), `sdk.ts` (`streamSimple`), `interactive-mode.ts`/`model-registry.ts` (`getProviders`) import from `@valkyriweb/pi-ai/compat`; type-only `Api`/`Model` stay on the root barrel. This is **not** a fork shim — upstream v0.80.2's own coding-agent src is identically on `/compat` (the `Models`-runtime migration is upstream's deferred work).
...
**Sequencing (revised — see decision below):** the baseline merge = v0.80.2 green is the **final** fork state. The originally-planned step-2 ("remove `/compat` from src, adopt the `Models` runtime") was **dropped**: investigation showed upstream's own coding-agent src is entirely on `/compat`, so removing it would diverge the fork *ahead* of upstream on the most active core files (`model-registry`/`sdk`/`compaction`/`auth-storage`) + require a risky fork-auth re-port, all thrown away when upstream ships its own migration. The fork keeps `/compat` to match upstream (zero divergence) and adopts the `Models` migration when upstream does.
