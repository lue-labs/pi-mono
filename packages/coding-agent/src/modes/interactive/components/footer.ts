...
export class FooterComponent implements Component {
	...
	private getUsageTotals(): UsageTotals {
		...
		for (let i = entries.length - 1; i >= 0; i--) {
			...
			if (entry?.type === "message" && entry.message.role === "assistant") {
				if (lastAssistantEntry === undefined) lastAssistantEntry = entry;
				else {
		...
		const cacheHitEligibleInput = totals.totalInput + totals.totalCacheRead;
		totals.averageCacheHitPct =
			cacheHitEligibleInput > 0 ? Math.round((totals.totalCacheRead / cacheHitEligibleInput) * 100) : 0;
	...
	render(width: number): string[] {
		...
		// cache writes. The footer's primary `cache N%` is the latest-turn cache hit
		// ratio: cacheRead / (input + cacheRead). Cache writes and large fresh tails
		// are health markers beside the metric; they must not dilute the hit ratio.
		...
		if (hasLatestUsage && lastUsage) {
			const health = computeCacheHealth({
				...
				previousAssistant: previousUsage
					? { usage: previousUsage, timestamp: previousTimestamp, model: previousModel ?? state.model?.id }
					: undefined,
			});
			...
			const label = [`cache ${health.hitPct}%`, `avg ${averageCacheHitPct}%`, ...markers].join(" ");
			let colored: string;
			...
			else if (health.hitPct >= 80) colored = theme.fg("success", label);
