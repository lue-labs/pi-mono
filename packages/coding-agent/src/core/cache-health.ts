...
export interface CacheHealthMetrics {
	...
	cacheRead: number;
	cacheWrite: number;
	/** Per-turn cache hit ratio: cacheRead / (input + cacheRead). Cache writes are warnings, not hit-ratio dilution. */
	hitPct: number;
	/** Total input coverage including cache writes. Kept for diagnostics/back-compat. */
	coveragePct: number;
	warmthPct: number;
...
export function computeCacheHealth(input: CacheHealthInput): CacheHealthMetrics {
	...
	const hitEligibleInput = freshInput + cacheRead;
	...
	const sameModel = input.model === undefined || previous?.model === undefined || input.model === previous.model;
	...
	return {
		...
		cacheRead,
		cacheWrite,
		hitPct: percent(cacheRead, hitEligibleInput),
