...
export function createAgentToolDefinition(
	...
	return {
		...
		promptGuidelines: [
			...
			"Before delegating, identify the critical path you should keep locally and any bounded sidecar tasks that can run in parallel. Do not delegate work that blocks your next step unless useful local work can proceed in parallel; while a child runs, do meaningful non-overlapping work, otherwise wait rather than manufacturing busywork.",
			"Trust but verify: a child agent's final message describes what it intended to do, not guaranteed ground truth. If a child edits code, runs commands, or claims gates passed, inspect the actual diff, files, output paths, or session transcript before reporting the work as done.",
			"Reach for this when the task matches one of the available agent types, when you have independent sidecar work to run in parallel, or when answering would mean reading across several files — delegate it and keep the conclusion, not the file dumps. For a single-fact lookup where you already know the file, symbol, or value, search directly with `read`/`grep`/`Glob`. Once you've delegated a search, don't also run it yourself — wait for the result and integrate it.",
			'Routing rule: **default to `explore`** for any read-only investigation (search, find, where/how/which, investigate, audit, map, trace). Choose `general` ONLY when the child itself must write files, run bash, or mix search+edit+verify in one run — "open-ended research" alone is not enough. If every step is read/grep/Glob/ls, it\'s `explore`. For coding work, prefer `worker` when the write set is known and bounded; use `explore`/`plan` first when scope discovery is still needed.',
			...
			"When parallel exploration or review is needed, send multiple `agent` tool-use blocks in one assistant message; Pi runs those calls concurrently. Use `tasks[]` only for explicit batched fan-out inside one agent call. Parallel code-edit children need non-overlapping write sets; overlapping edits must be serialized, and shared imports/build/codegen targets can still collide even when file paths differ.",
