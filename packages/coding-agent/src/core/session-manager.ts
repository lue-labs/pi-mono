...
import type { ImageContent, Message, TextContent, Usage } from "@valkyriweb/pi-ai";
...
export class SessionManager {
	...
	setSessionFile(sessionFile: string, hydrationOptions: SessionHydrationOptions = {}): void {
		...
		if (existsSync(this.sessionFile)) {
			...
			let headerBeforeLoad: SessionHeader | null = null;
			try {
				headerBeforeLoad = readSessionHeader(this.sessionFile);
			} catch (error) {
	...
	static open(
		...
		if (cwdOverride === undefined && existsSync(resolvedPath)) {
			try {
				...
				if (!(error instanceof SessionHeaderScanLimitError)) throw error;
				// The bounded scan only serves discovery. Fully load legacy sessions with
				// very large headers or prefixes so opening them remains compatible.
				const firstEntry = loadEntriesFromFile(resolvedPath)[0];
