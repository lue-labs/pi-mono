...
```
Warning: Skipped extension "/repo/.pi/extensions/redraws.ts": Cannot find module '@lue-labs/pi-tui'
```
...
| Package | Purpose |
|---------|---------|
| `@lue-labs/pi-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@lue-labs/pi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@lue-labs/pi-tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@lue-labs/pi-coding-agent";
...
```typescript
import { isBashToolResult } from "@lue-labs/pi-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@lue-labs/pi-coding-agent";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@lue-labs/pi-coding-agent";
...
```typescript
import { SessionManager } from "@lue-labs/pi-coding-agent";
...
```typescript
import type { AutocompleteItem } from "@lue-labs/pi-tui";
...
and behavior through `onChange`. `currentValue` may be a string or a getter
so reopening the modal can reflect an in-session change. The `extensionPath`
field is added by pi when it exposes registered settings to the selector.
...
project-scoped configuration continues to take precedence over global values.
...
```typescript
import { withFileMutationQueue } from "@lue-labs/pi-coding-agent";
...
**Important:** Use `StringEnum` from `@lue-labs/pi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
- [glob.ts](https://github.com/earendil-works/pi-mono/blob/main/packages/coding-agent/src/core/tools/glob.ts) - `GlobToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@lue-labs/pi-coding-agent";
...
```typescript
import { createBashTool } from "@lue-labs/pi-coding-agent";
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API. Default-shell rows are composed by [`tool-execution.ts`](../src/modes/interactive/components/tool-execution.ts) and [`tool-panel.ts`](../src/modes/interactive/components/tool-panel.ts).

By default, tool output is wrapped in a compact panel with two cells of horizontal padding, no vertical padding, lifecycle background color, and strict terminal-width clamping. At terminal widths below five cells, the panel reduces its side padding so content still fits. A defined `renderCall` or `renderResult` must return a `Component` and should respect the width passed to its `render()` method. If a slot renderer is not defined, `tool-execution.ts` uses fallback rendering for that slot.

Set `renderShell: "self"` when the tool should render its own shell instead of using the default panel. This is useful for tools that need complete control over framing or background behavior, for example large previews that must stay visually stable after the tool settles.
...
```typescript
import { keyHint } from "@lue-labs/pi-coding-agent";
...
- Use `Text` with padding `(0, 0)`. The default panel handles horizontal padding and lifecycle backgrounds.
...
- Use `renderShell: "self"` only when the default panel gets in the way. In self-shell mode the tool is responsible for its own framing, padding, width clamping, and background.
...
```typescript
import { Text, Component } from "@lue-labs/pi-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@lue-labs/pi-coding-agent";
import { matchesKey } from "@lue-labs/pi-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@lue-labs/pi-coding-agent";
