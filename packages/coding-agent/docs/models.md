...
Use `thinkingLevelMap` on a model to describe model-specific thinking controls. Keys are pi thinking levels: `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`, `adaptive`. Maps may contain holes; for example, a model can expose `high` and `max` without exposing `xhigh`. Ultra is opt-in and maps to the model's maximum native effort; OpenAI Responses sends it as `max`.
