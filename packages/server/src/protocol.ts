import {
	...
	type ToolReferenceContent as AiToolReferenceContent,
...
function toProtocolAssistantContent(message: AssistantMessage): AssistantTranscriptItem["content"] {
	return message.content.map((part) => {
		switch (part.type) {
			...
			case "tool_reference":
				return { type: "tool_reference", name: identifier(part.name, "Tool reference name") };
			default: {
				const exhaustive: never = part;
				return exhaustive;
			}
...
function toProtocolToolContent(
	content: Array<AiTextContent | AiImageContent | AiToolReferenceContent>,
