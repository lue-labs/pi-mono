...
- Fix: follow models.dev's rename of the Kimi For Coding provider (`kimi-for-coding` → `kimi-code-plan-global`, plus a new `kimi-code-plan-cn`). The generator hardcoded the old key, so the lookup matched nothing and emitted zero `kimi-coding` models while `src/models.generated.ts` still imported that provider. `hydrate:model-data --strict` then aborted with `Cannot hydrate missing providers: kimi-coding`, wrote no provider JSON, and `build:offline` surfaced the confusing downstream `TS2307: Cannot find module './kimi-coding.models.ts'`. This broke every clean checkout while existing working copies kept building from gitignored pre-rename data. The source key is now resolved across known names, keeping the internal `kimi-coding` id and `api.kimi.com/coding` base URL so generated output is unchanged; an unresolvable key now logs a warning naming the constant to update, while the existing hydrate guard still fails the build when the generated aggregator actually demands the provider.
