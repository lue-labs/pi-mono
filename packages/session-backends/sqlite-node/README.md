...
Node `node:sqlite` Session backend for `@lue-labs/pi-agent-core`.

```ts
import { BACKGROUND_CONTEXT } from "@lue-labs/pi-agent-core";
import {
  createNodeSqliteFactory,
  SqliteSessionRepo,
} from "@lue-labs/pi-session-backend-sqlite-node";
